...
@router.get("/{category}", response_model=List[Place])
...
@router.get("/{category}/timeline/operating")
...
@router.post("/{category}", response_model=ResponseMessage)
def api_add_place(category: str, place: Place):
    add_place(category, place)
    return {"message":f"'{category}' 에 장소가 추가되었습니다."}

@router.put("/{category}/{place_id}", response_model=ResponseMessage)
...
@router.delete("/{category}/{place_id}", response_model=ResponseMessage)
